...
// changed the source code of MSApriori ALgorithm from SPMF library
//changes in the source code for item constraint,Multiple minimum support and Item Constraint
//are maintained with CS19MDS11017 comments
...
//CS19MDS11017 lists for storing items sets of must contain sets and cannot contains sets
	List<Integer> must_contain =new ArrayList<Integer>();
	List<Integer[]> cannot_contain = new ArrayList<Integer[]>();
	/**
	 * Constructor
	 */
	public Exercise_3() {
	 ...
	 */
	//CS19MDS11017 Si: Parameter for Support difference constraint
	public void runAlgorithm(String input, String output,
			double beta, double LS,double si, Integer[][] must, Integer[][] cannot) throws IOException { 
...
//		CS19MDS11017 :Adding the each item set to list
		
		for (Integer[] arr : must){
	        for(int i=0 ; i<arr.length ; i++){
	            must_contain.add(arr[i]);
	        }
	    }
		...
		for (Entry<Integer, Integer> entry : mapItemCount.entrySet()) {
			...
			if (entry.getValue() >= MIS[entry.getKey()] && maxItemsetSize >=1) {
				...
				//CS19MDS11017 Printing item sets from must contain sets of length 1
				if(must_contain.contains(entry.getKey())){
		...
		//CS19MDS11017 : calculating support difference paramter value 
		si = (si * transactionCount);
		//CS19MDS11017 :for each cannot contain itemset ,joining with other cannot contain itemset
		//and creating a list of all possible combinations of cannot contain itemsets
		for (i=0;i<cannot.length;i++) {
			for(int j = i+1 ; j<cannot.length;j++) {
				Integer[] result = new Integer[cannot[i].length + cannot[j].length];
				System.arraycopy(cannot[i], 0, result, 0,cannot[i].length );
				System.arraycopy(cannot[j], 0, result , cannot[i].length, cannot[j].length);
				Object[] x =  Arrays.stream(result).distinct().toArray();
				result = Arrays.copyOf(x, x.length, Integer[].class);
				Arrays.sort(result,itemComparator);
				cannot_contain.add(result);
			}
		}
		...
		if(maxItemsetSize >1){
			...
			do {
				...
				if (k == 2) {
					...
					candidatesK = generateCandidate2(F, mapItemCount,si);
				} else {
					// otherwise, we use the general procedure for candidate generation
					candidatesK = generateCandidateSizeK(level,mapItemCount,si);
				}
	...
	private List<Itemset> generateCandidate2(List<Integer> frequent1, Map<Integer, Integer> mapItemCount
		...
		for (int i = 0; i < frequent1.size(); i++) {
			...
			for (int j = i + 1; j < frequent1.size(); j++) {
				...
				//CS19MDS11017 checking if any items are present in must contain itemsets
				if(must_contain.contains(item1) | must_contain.contains(item2)) {
				//CS19MDS11017 : if the difference between both items is less than
					//support difference constraint,create the candidate
				if ((mapItemCount.get(item2)>= MIS[item1]) & 
					((Math.abs(mapItemCount.get(item2)-mapItemCount.get(item1))) <= si) ) {
				// Create a new candidate by combining itemset1 and itemset2
					//CS19MDS11017:function to check cannot contain itemsets list
					if (cannot_contain_list(new int[] { item1, item2 })) {
	...
	//CS19MDS11017:function to check if newly created candidates itemsets contain cannot contain sets
	public boolean cannot_contain_list(int[] candidate) {
		Integer[] arr = Arrays.stream(candidate).boxed().toArray(Integer[]::new);
		int bool =0;
		boolean p =true;
		for (int i = 0;i<cannot_contain.size();i++) {
			HashSet<Integer> set = new HashSet<>(); 
			set.addAll(Arrays.asList(cannot_contain.get(i)));
			set.retainAll(Arrays.asList(arr));
			
			if (set.size() == cannot_contain.get(i).length)
			{ bool++ ; break;}
		}
		if (bool > 0) {p = false;}
		return p;	
	 ...
	 */	
	protected List<Itemset> generateCandidateSizeK(List<Itemset> levelK_1,Map<Integer, Integer> mapItemCount,
			double si) {
		...
		loop1: for (int i = 0; i < levelK_1.size(); i++) {
			...
			loop2: for (int j = i + 1; j < levelK_1.size(); j++) {
				...
				//CS19MDS11017 : if the difference between both items is less than
				//support difference constraint,create the candidate
				if(Math.abs(mapItemCount.get(itemset2[itemset2.length - 1]) -
						    mapItemCount.get(itemset1[itemset1.length - 1])) <= si ) {
				// Create a new candidate by combining itemset1 and itemset2
				int newItemset[] = new int[itemset1.length + 1];
				System.arraycopy(itemset1, 0, newItemset, 0, itemset1.length);
				newItemset[itemset1.length] = itemset2[itemset2.length - 1];
				//CS19MDS11017 :functiom to check cannot contain itemsets
				if(cannot_contain_list(newItemset)) {
